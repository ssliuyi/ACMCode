...
void Search(int k,int p)//对于字母k 循环p次
{
    ...
    for(int a=0;a<k;a++)
    {
        for(int i=0;i<p;i++)
        {
            ...
            if(rect[a].right<x) rect[a].left=x;
        }
    }
...
bool judge()//对于字母k 循环p次
{
    ...
    {
        for(j=0;j<k;j++)
            if(rect[i].top-rect[j].bottom>=0&&(i<j)) mark[rect[i].rank][rect[j].rank]=1;
    }
    ...
    {
        for(j=0;j<k;j++)
            if(i<j && rect[i].right-rect[j].left>=0 && mark[rect[i].rank][rect[j].rank]==1) return false;
    }
    return true;
}
