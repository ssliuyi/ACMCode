...
bool judge()//对于字母k 循环p次
{
    ...
    {
		for(j=0;j<k;j++)
			if(i<j && rect[i].bottom-rect[j].top>=0) {mark[rect[i].rank][rect[j].rank]=1; mark[rect[j].rank][rect[i].rank]=1;}
    }
    sort(rect,rect+32,compare2);
    for(i=0;i<k;i++)
    {
		for(j=0;j<k;j++)
			if( i<j && rect[i].right-rect[j].left>=0 && (mark[rect[i].rank][rect[j].rank]==1||mark[rect[j].rank][rect[i].rank]==1)) return false;
    }
    return true;
}
